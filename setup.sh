...
sudo mv "./vulcano-$VULCVERSION/vulcanod" /usr/local/bin
sudo mv "./vulcano-$VULCVERSION/vulcano-cli" /usr/local/bin
sudo mv "./vulcano-$VULCVERSION/vulcano-tx" /usr/local/bin
...
until sudo su -c "vulcano-cli getinfo 2>/dev/null | grep -q 'version'" vulcano; do
  sleep 1;
done
...
until sudo su -c "vulcano-cli mnsync status 2>/dev/null | grep '\"IsBlockchainSynced\" : true' > /dev/null" vulcano; do
  echo -ne "Current block: "$(sudo su -c "vulcano-cli getinfo | grep blocks | awk '{print $3}' | cut -d ',' -f 1)'\\r'") vulcano
  sleep 1
done

# Unlock wallet
until sudo su -c "vulcano-cli getstakingstatus | grep walletunlocked | grep true" vulcano; do
...
log in with the vulcano user and the password you set earlier.
